...
namespace BigIntegerSum
{
    ...
    internal class BigIntegerSum
    {
        private static void Main()
        {
            int[] integerOne =
                {
                    1, 2, 9, 9, 9, 5, 2, 0, 0, 1, 2, 5, 4, 4, 5, 1, 4, 3, 2, 4, 0, 1, 2, 5, 4, 4, 5, 1, 4, 
                    3, 2, 4
                };
        ...
        private static List<int> SumNumbers(int[] a, int[] b)
        {
            // The sum numbers is done as we do it on paper
            Array.Reverse(a);
            Array.Reverse(b);
            var memory = 0; // This will be used to carry over the surplus from each digit being added.
            int biggerArray;
            int smallerArray;
