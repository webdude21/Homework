...
namespace ExtractDates
{
    ...
    internal class ExtractDates
    {
        ...
        {
            var pattern = @"\b(0[1-9]|[12][\d]|3[01]|\d).(0[1-9]|1[012]|\d)\.(19|20)[\d]{2}";
            var input =
                "Some dates can be written like so 1.2.2013 others can be written like so 01.05.2013, dates like 41.13.1813 will not be passed as valid.";
            ...
            foreach (Match date in matches)
            {
                var currentDate =
                    DateTime.ParseExact(date.ToString(), dateFormat, CultureInfo.InvariantCulture)
                        .ToString(new CultureInfo("en-CA").DateTimeFormat.ShortDatePattern);
