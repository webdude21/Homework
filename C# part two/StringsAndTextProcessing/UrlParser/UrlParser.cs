...
namespace UrlParser
{
    ...
    internal class UrlParser
    {
        ...
        {
            var input = "ftp://www.devbg.org/forum/index.php";
        ...
        private static void GetURLEasy(string input)
        {
            var uri = new Uri(input);
            Console.WriteLine(
                "This is the protocol: '{0}'\r\nThis is the server: '{1}'\r\nThis is the resource: '{2}'", 
                uri.Scheme, 
                uri.Host, 
                uri.AbsolutePath);
        }

        private static void GetURLRegEX(string input)
        {
            var result = Regex.Match(
                input, 
                @"(?<protocol>https?|ftp)://(?<server>[^/]+)/(?<resource>.*)", 
                RegexOptions.IgnoreCase);
            Console.WriteLine(
                "This is the protocol: '{0}'\r\nThis is the server: '{1}'\r\nThis is the resource: '{2}'", 
                result.Groups["protocol"].Value, 
                result.Groups["server"].Value, 
                result.Groups["resource"].Value);
