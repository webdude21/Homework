...
namespace UpCaseTags
{
    using System;
    using System.Text.RegularExpressions;

    internal class UpCaseTags
    {
        private static void Main()
        {
            ...
            var expression = new Regex(@"<upcase>(?<toUp>[^<]+)</upcase>", RegexOptions.IgnoreCase);
            var output = expression.Replace(input, k => k.Groups["toUp"].Value.ToUpper());
